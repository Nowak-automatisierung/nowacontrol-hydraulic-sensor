...
| EM2 | Deep sleep | Off | On (LFXO) | ~2–10 µA | Primary idle state |
| EM3 | Stop | Off | Off | ~1 µA | Long intervals only |
...
- Average current target: **< 50 µA** — unvalidated until M3 bench measurement.
...
- Bootloader + 2× application slots must fit in 1536 kB flash.
...
- [ ] Average sleep current measured (target: < 50 µA)
