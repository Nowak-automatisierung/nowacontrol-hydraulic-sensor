...
- LDO efficiency is low at light load (typical quiescent ~55 uA for XC6220-class LDO), which adds
...
- [ASSUMPTION] Pressure sensor with I2C or SPI digital interface: quiescent ~0.5-2 mA during active
  measurement, ~1 uA during powerdown.
...
| EM2 (deep sleep) | ~2 uA | Yes | Yes |
| EM3 (stop) | ~1.5 uA | Yes | Yes |
| EM4 (shutoff) | ~0.3 uA | No | No |
...
- **Recommendation:** Implement GPIO-controlled sensor power switch before estimating realistic sleep
  current. Verify with uA bench (Section 7, measurement M2).
...
- Battery life estimate (rough order-of-magnitude, [ASSUMPTION] all values):
  ...
  | Sleep current (EM2 + sensor off) | 5 uA total |
  | Active cycle current | 18 mA average |
  | Active cycle duration | 50 ms per wakeup |
  | Wakeup interval | 60 s |
  | Average current | ~5 uA + (18 mA x 50 ms / 60 s) = ~5 uA + 15 uA = ~20 uA |
...
- At 2.4 GHz, λ/4 approx. 31 mm in free space. Metal objects within λ/4 of the antenna will have
...
|---|---|---|---|---|
| M1 | VDD rail droop during Zigbee TX burst | Brownout risk assessment | Oscilloscope on 3.3 V rail, trigger on RF TX enable GPIO or RAIL TX event; 10 us/div | < 100 mV droop from nominal; no brownout reset observed |
| M2 | Sleep current (MCU in EM2/EM3, sensor off) | Battery life baseline | uA bench (e.g., PPK2 or Otii Arc) with no charger connected, sensor GPIO held low | < 5 uA total (MCU + LDO quiescent + any peripheral leakage) |
...
- [ ] **Sensor rail power switching implemented and verified:** Sleep current measured (M2/M3)
      with sensor rail switching in firmware; result meets < 5 uA target or target is revised
